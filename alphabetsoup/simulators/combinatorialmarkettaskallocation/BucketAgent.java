...
*/
	...
	public void rebidOnWordStationLetterRequests() {
		oldRebidOnWordStationLetterRequests();
		ComplexWordStationOffer co = new ComplexWordStationOffer();
		co.bucket = this;
		
		SimulationWorldMarketTaskAllocation sw = SimulationWorldMarketTaskAllocation.getSimulationWorld();
		if(assignedBucketbot != null) {
			co.bucketbots.add(assignedBucketbot);
			co.bucketbotCosts.add(0.0f);
		}
		else { //need to get one
			for(BucketbotAgent ba : sw.bucketbotAgents) {
				if(ba.reservedBucket != null)
					continue;
				co.bucketbots.add(ba);
				co.bucketbotCosts.add(ba.bucketbot.getDistance(this));				
			}
		}
		
		for(Letter l : unassignedLetters)
			co.letterOffers.add(new ComplexWordStationOffer.LetterOffer(l, .5f));		
		
		Economy economy = SimulationWorldMarketTaskAllocation.getSimulationWorld().economy;
		economy.wordStationOffers.put(this, co);
	}
	
	public void oldRebidOnWordStationLetterRequests() {
		Economy economy = SimulationWorldMarketTaskAllocation.getSimulationWorld().economy;
