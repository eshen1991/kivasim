...
public class LetterManagerExample implements Updateable {
	...
	public void update(double last_time, double cur_time) {
		for(LetterStation s : SimulationWorldGraphExample.getSimulationWorld().getLetterStations()) {
			...
			if(s.getAssignedLetters().size() < s.getCapacity()) {
				...
				//s.addLetter(SimulationWorldGraphExample.simulationWorld.wordList.generateRandomLetter());
				...
				if(requiredLetters.size() > 0) {
					...
					s.addBundle(l);
					SimulationWorldGraphExample.getSimulationWorld().bucketbotManager.newLetterBundleAssignedToStation(l, s);
				}
			}
		}
	}
